...
c ... directly extract relative euler angles from multiplied matrix
      do i=1,3
        do j=1,3
          rotma2(i,j)=0.d0
          do k=1,3
            rotma2(i,j)=rotma2(i,j)+rotma1(k,i)*rotmat(k,j) ! relative euler angles: rotma1*rotmat
          enddo
        enddo
      enddo

      cost=rotma2(3,3) ! the (3,3) in rotmatrix is costheta
      call within(cost)
      theta2=acos(cost)
...
c ... theta=0
        phi2=0.d0 ! if phi2 and theta2 is zero then (1,1) in rotmatrix is coschi only
        cchi=rotma2(1,1)
        schi=rotma2(2,1) ! maybe a negative sign before
        call within(cchi)
        call within(schi)
...
c ... theta=pi
        phi2=0.d0
        cchi=rotma2(2,2) ! if theta is pi, and phi is zero, then (2,2) in rotmatrix is coschi only
        schi=rotma2(1,2)
        call within(cchi)
        call within(schi)
        if(schi.gt.zero) then
...
! the Euler angles in rotmatrix2 is the relative Euler angles between 1 and 2
      Eulrel(1)=phi2
      Eulrel(2)=theta2
      Eulrel(3)=chi2
...
! Generate Rotational matrix by
      ...
      rotmat(1,2)=-cp*ct*sk-sp*ck  ! Different from the rotmatrix in mcpiqmc
      rotmat(1,3)=cp*st
...
! rottrn() is to transform from WFF to SFF
      do i=1,3
        rsf(i)=rcom(i) ! rsf: rot spaced frame? rwf: rot water fixed frame?
        do j=1,3
...
! As rotational matrix is orthogonal, the transpose of rotmat is the inverse of it
      do i=1,3
        do j=1,3
...
c ... RATTLE and SHAKE formula
     ...
     +          a2bohr=1.8897162d0,wno2k=0.6950356d0) ! it is units conversion
